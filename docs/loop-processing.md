...
- **Out:** moving a loop when its backing track moves, a staged-copy
  management screen. See [Deferred](#deferred).

## Constraints that shaped this

- **`BT.WAV` is one-directional (host to pedal) and re-derivable.** It has a
...
the loop*. It means nothing accumulates on the Pi, and there is no
time-based prune to get wrong. Staging (rather than streaming the pedal
...
- **Download never deletes.** Removing a loop is a distinct action the user
  takes deliberately.
- Guarded by a confirm: *"Delete this loop from the pedal? You can't undo
  this."* That catches the accidental click, and nothing more is warranted.
- Because download never deletes and delete is separate, a failed or partial
  download can never cost a loop. Completion is not wired to deletion, so it
  does not matter that a browser download cannot be verified from here.
...
showing *less* free, which is the safe direction.
...
loop-bearing slots; refresh after a loop delete. Do not `stat` 99 directories on every snapshot. `_emit` runs often (SSE), and 99 stats over 1 MB/s USB per
...
- A **Download** affordance on a loop-bearing slot → `GET /api/loops/<slot>` →
  browser download. "Leave behind" is implicit: download changes nothing on the
  pedal.
- A **Remove loop** affordance → confirm dialog → `DELETE /api/loops/<slot>`.
- Naming throughout is "Loops" and "Download loops", never "backup". The tool
  can delete an irreplaceable take, and "backup" would imply it cannot.
...
2. **Loop-vs-move coupling.** `move`/`swap` operate on `BT.WAV` only; a
   ...
   *does not* move its loop, and after a move a slot's loop may belong to a
   ...
   move/swap carry the loop. Not decided.
