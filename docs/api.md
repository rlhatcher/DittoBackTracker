...
| `update_available` | `true` when the update check found the tracked remote branch ahead of the deployed commit. The check runs at startup and then every `DITTO_UPDATE_CHECK_SECS` (default 3600). `false` until the first check completes, if it's up to date, or if the check couldn't run (offline, no checkout) |
